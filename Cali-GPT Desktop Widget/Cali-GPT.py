#Cali-GPT v10 coding bot for Discord
# Made by CaliNux
# 6/17/23

# import discord - is discord bot
import discord
# import openai - uses for requests
import openai
# import logging so we can log usernames, prompts, and responses. to allow us to better track error processing, troubleshooting, etc.
import logging
# datetime used to keep track of non-subbers and to only allow them limited API use
from datetime import datetime, timedelta, date
# import asyncio - used for deleting old threads
import asyncio
from discord import Thread

# Default stuff section
...
async def on_message(message):
    ...
    if not has_new_role:
        if user_id in user_requests:
            ...
            if datetime.now() - last_request_time < COOLDOWN_DURATION:
                ...
                return
                # this is charging free users to use the help command and i literally cant find a way around it. maybe slash commands?
    prompt = message.content.replace(f"<@{client.user.id}>", "").strip()

    if not has_new_role:
        user_requests[user_id] = datetime.now()

    # Check if the user wants help or whatever
    ...
    for language, (language_tag, language_name) in languages.items():
        if language in prompt_lower:
            # Remove the language tag from the prompt
            prompt = prompt.replace(language, "").strip()

            # Include the fixed text before the user's query
